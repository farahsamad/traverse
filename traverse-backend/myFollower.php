<?php
    ...
    if(isset($_POST['id'])){
        ...
        $querymyfollower="SELECT count(follower_id) FROM follow WHERE following_id='$id'  AND follower_request='1' AND following_request='0'";
        $resultmyfollower=mysqli_query($con,$querymyfollower);
        while($rowmyfollower=mysqli_fetch_assoc($resultmyfollower)){
            $countmyfollower=$rowmyfollower['count(follower_id)'];
            // echo"countfollower".$countfollower;

        }
        $searchmyfollowerid="SELECT * FROM follow WHERE following_id='$id' AND follower_request='1' AND following_request='0'";
        $resultsearchmyfollowerid=mysqli_query($con,$searchmyfollowerid);
        while($rowmyfollowerid=mysqli_fetch_assoc($resultsearchmyfollowerid)){
            $followingid=$rowmyfollowerid['follower_id'];
            $FID=$rowmyfollowerid['FID'];
            $searchmyfollower="SELECT * FROM user WHERE ID='$followingid' ";
            $resultsearchmyfollower=mysqli_query($con,$searchmyfollower);
            while($rowmyfollowername=mysqli_fetch_assoc($resultsearchmyfollower)){
                $followername=$rowmyfollowername['username'];
                $followerimage=$rowmyfollowername['profile'];
                $followerbio=$rowmyfollowername['bio'];
                $followerclass= new stdClass();
                $followerclass->followername=$followername;
                $followerclass->followerimage=$followerimage;
                $followerclass->followerbio=$followerbio;
                $followerclass->FID=$FID;
                $followerclass->followingid=$followingid;
                array_push($myfollower_result, $followerclass);
            }
        }
        ...
        $myfollowerArray= new stdClass();
        $myfollowerArray->followersname=$myfollower_result;
        $myfollowerArray->myfollowerscount=$countmyfollower;
        array_push($response_result, $myfollowerArray);
