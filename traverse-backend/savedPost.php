<?php
    ...
    $_POST= json_decode(file_get_contents("php://input"),true);
                    ...
                    $select="SELECT*FROM saves WHERE ID='$id'";
                    $selectresult=mysqli_query($con,$select);
                    while($selectfetch=mysqli_fetch_assoc($selectresult)){
                        $savePID = $selectfetch['PID'];
                        $search="SELECT*FROM userpost  WHERE PID='$savePID'  ORDER BY time DESC";
                        ...
                        while($fetch=mysqli_fetch_assoc($res)){
                            ...
                            $liked=0;
                            $searchforlike="SELECT * FROM likes WHERE ID='$id' AND PID='$savePID'";
                            ...
                            $searchforuser="SELECT * FROM user WHERE ID='$userID' ";
                            $userresult=mysqli_query($con,$searchforuser);
                            while($userrow=mysqli_fetch_assoc($userresult)){
                                $followingpic= $userrow['profile'];
                                $followingname= $userrow['username'];
                                $followingbio= $userrow['bio'];                                       
                            }

                            $comment_response=array();
                            $searchforlike="SELECT * FROM comments WHERE PID='$savePID'";
                            $likeresult=mysqli_query($con,$searchforlike);
                            while($likesrow=mysqli_fetch_assoc($likeresult)){
