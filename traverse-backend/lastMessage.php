    ...
    if(isset($_POST['id'])){
        ...
        while($fetch=mysqli_fetch_assoc($res)){
            ...
            if($senderId == $id){
                ...
                $sendby = $senderId;
            $fetchMessage="SELECT COUNT(message_status),ID,RID FROM message WHERE ID='$sendby' AND RID='$id' AND message_status='unread' ";
            $satustCount = mysqli_query($con,$fetchMessage);
            while($count=mysqli_fetch_assoc($satustCount)){
                $unreadCount = $count['COUNT(message_status)'];
                $ID = $count['ID'];
                $RID = $count['RID'];
